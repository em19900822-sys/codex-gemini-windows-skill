...
维护日期：2026-09-22。安装前重新检查目标电脑与上游变化，不把历史版本描述为最新。
...
- **本机故障与恢复实测（2026-09-22）**：系统代理已切换为 `127.0.0.1:23054`，两项后台进程仍继承 `127.0.0.1:21761`；旧端口未监听。重载已核对身份的后台后，GPT-5.6 Luna 与 Gemini 3.8 Flash 通过 Codex 临时后台协议的实际回复测试。本轮尚未取得用户对原报错桌面任务的重新发送确认。
- **自动端口跟随（2026-09-22）**：在本机已有启动器上增加端口变化检测和恢复回调，已做隔离状态变化测试并确认隐藏后台持续运行；未故意切断用户正在工作的网络，也未用本轮测试证明所有休眠、断网或重新登录情形都已解决。公开 Skill 记录可复用的实施与验证要求，不复制本机凭据、进程内存、运行日志或专用启动包；详见 [proxy-port-recovery.md](proxy-port-recovery.md)。
